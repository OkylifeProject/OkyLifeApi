...
<div class="fieldcontain ${hasErrors(bean: eatActivityInstance, field: 'okiBar', 'error')} ">
    <label for="okiBar">
        <g:message code="eatActivity.okiBar.label" default="Oki Bar"/>

    </label>
    <g:select id="okiBar" name="okiBar.id" from="${okylifeapi.OkiBar.list()}" optionKey="id"
              value="${eatActivityInstance?.okiBar?.id}" class="many-to-one" noSelection="['null': '']"/>
