...
<div class="fieldcontain ${hasErrors(bean: okiBarInstance, field: 'associatedActivity', 'error')} required">
	<label for="associatedActivity">
		<g:message code="okiBar.associatedActivity.label" default="Associated Activity"/>
		<span class="required-indicator">*</span>
	</label>
	<g:select id="associatedActivity" name="associatedActivity.id" from="${okylifeapi.Activity.list()}" optionKey="id"
			  required="" value="${okiBarInstance?.associatedActivity?.id}" class="many-to-one"/>
