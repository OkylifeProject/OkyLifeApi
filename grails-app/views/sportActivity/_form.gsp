...
<div class="fieldcontain ${hasErrors(bean: sportActivityInstance, field: 'okiBar', 'error')} ">
    <label for="okiBar">
        <g:message code="sportActivity.okiBar.label" default="Oki Bar"/>

    </label>
    <g:select id="okiBar" name="okiBar.id" from="${okylifeapi.OkiBar.list()}" optionKey="id"
              value="${sportActivityInstance?.okiBar?.id}" class="many-to-one" noSelection="['null': '']"/>
